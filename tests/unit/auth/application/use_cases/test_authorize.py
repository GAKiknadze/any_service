...
from src.auth.application.dtos import AccessTokenRequestDTO, UserResponseDTO
from src.auth.application.use_cases import AuthorizeUseCase
from src.auth.domain.entities import AuthSession, AuthUser


@pytest.mark.asyncio
async def test_authorize_success(fake_user_repo, fake_session_repo, fake_token_service):
    user_id = uuid4()
    session_id = uuid4()
    ...
    use_case = AuthorizeUseCase(fake_session_repo, fake_token_service)
